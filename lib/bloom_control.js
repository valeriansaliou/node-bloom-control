...
BloomControl.prototype.connect = function(handleSuccess, handleError) {
  ...
  if (self.__client === null) {
    ...
    try {
      ...
      // Generate connect options
      var connectOptions = {};

      if (self.__options.socket) {
        connectOptions.path = self.__options.socket;
      } else {
        connectOptions.port = self.__options.port;
        connectOptions.host = self.__options.host;
      }
